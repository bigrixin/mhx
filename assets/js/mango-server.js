...
const DataSchema = new mongoose.Schema({
  ...
  // 卦象起卦方法
  method: String,        // 起卦方式 (manual/number/random/time)
  guaMethodText: String, // 起卦方法文字描述
  
  // 日期时间信息
  dateTime: String,      // ISO格式的完整日期时间
  
  // 农历信息
  lunarDate: String,     // 农历日期
  ganZhi: String,        // 干支（月日）
  nianGanZhi: String,    // 年干支
  jieQi: String,         // 节气信息
  kongWang: String,      // 旬空信息
  
  // 卦象数据 - 使用 Mixed 类型支持复杂嵌套对象
  gua: mongoose.Schema.Types.Mixed,        // 卦象对象（original/changed/palaceInfo等）
  guaDetails: mongoose.Schema.Types.Mixed, // 卦象详细信息
  
  // 时间戳
  createdAt: { type: Date, default: Date.now },
  updatedAt: { type: Date, default: Date.now }
}, { strict: false }); // 使用 strict: false 接受未定义的字段
...
// 接口10：获取 Collection 的字段结构
app.get('/api/collection-schema/:collectionName', async (req, res) => {
  try {
    ...
    // 转换为数组，定义字段顺序（常用字段优先）
    const commonFields = ['name', 'time', 'period', 'gender', 'category', 'description', 'method', 'dateTime'];
    let fields = [];
    ...
    // 再添加其他字段
    const otherFields = Array.from(fieldsSet).sort();
